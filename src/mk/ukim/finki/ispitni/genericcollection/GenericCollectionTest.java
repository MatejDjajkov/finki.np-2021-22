package mk.ukim.finki.ispitni.genericcollection;
...
class GenericCollection< T extends IHasTimestamp & Comparable<T>>
{
    Map<String,Set<T>> elementsByCategory;

    public GenericCollection() {
        elementsByCategory=new TreeMap<>();
    }

    public void addGenericItem(String category, T element) {
        elementsByCategory.putIfAbsent(category,new TreeSet<>());
        elementsByCategory.get(category).add(element);
    }
    public Collection<T> findAllBetween (LocalDateTime from, LocalDateTime to)
    {
        return elementsByCategory.values()
                .stream()
                .flatMap(Collection::stream)
                .filter(i->i.getTimestamp().isAfter(from)&&i.getTimestamp().isBefore(to))
    ...
    {
        return elementsByCategory.keySet()
                .stream()
                .filter(categories::contains)
                .map(i->elementsByCategory.get(i))
                .flatMap(Collection::stream)
                .collect(Collectors.toCollection(()->new TreeSet<T>(Comparator.reverseOrder())));
    }
    public Map<String, Set<T>> byMonthAndDay()
    {
        return elementsByCategory.values()
                .stream()
                .flatMap(Collection::stream)
                .collect(Collectors.groupingBy(i->i.getTimestamp().toString().substring(5,10),
                        ()->new TreeMap<>(),
